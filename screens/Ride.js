...
export default class RideScreen extends Component {
  ...
  handleTransaction = async () => {
    var { consoleId, userId, email } = this.state;
    ...
    if (!transactionType) {
      ...
      var isEligible = await this.checkUserEligibilityForStartConsole(
        userId,
        email
      );
      ...
      if (isEligible) {
        ...
        this.assignConsole(consoleId, userId, consoleType, userName, email);
        Alert.alert(
          "You have rented the console for next 1 hour. Enjoy your console!!!"
        );
        this.setState({
      ...
      var isEligible = await this.checkUserEligibilityForEndConsole(
        consoleId,
        userId,
        email
      );
      ...
      if (isEligible) {
        var { consoleType, userName } = this.state;
        this.returnconsole(consoleId, userId, consoleType, userName, email);
        ...
        // For Android users only
        // ToastAndroid.show(
        //   "We hope you enjoyed your console",
        //   ToastAndroid.SHORT
        // );
      }
    }
  ...
  checkUserEligibilityForStartConsole = async (userId, email) => {
    ...
    if (userRef.docs.length == 0) {
      ...
      userRef.docs.map(doc => {
        if (!doc.data().console_assigned) {
          ...
          isUserEligible = false;
          Alert.alert("End the current console to rent another console.");
          this.setState({
            consoleId: ""
          });
        }
  ...
  checkUserEligibilityForEndConsole = async (consoleId, userId, email) => {
  ...
  assignConsole = async (consoleId, userId, consoleType, userName, email) => {
    //add a transaction
  ...
  returnConsole = async (consoleId, userId, consoleType, userName, email) => {
    //add a transaction
    db.collection("transactions").add({
      user_id: userId,
      user_name: userName,
        ...
        <View style={styles.lowerContainer}>
          ...
          >
            <Text style={styles.buttonText}>
              {consoleAssigned ? "End Console" : "Unlock"}
            </Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
